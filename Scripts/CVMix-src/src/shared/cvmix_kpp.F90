...
! !DEFINED PARAMETERS:
  ...
  integer, parameter :: LANGMUIR_MIXING_DNN1         = 3
  ...
  integer, parameter :: LANGMUIR_ENTRAINMENT_DNN2    = 4
...
! !PUBLIC MEMBER FUNCTIONS:
  ...
  public :: cvmix_kpp_compute_bulk_Richardson1
  public :: cvmix_kpp_compute_turbulent_scales
  public :: cvmix_kpp_compute_unresolved_shear
  public :: cvmix_kpp_compute_unresolved_shear1
...
! !PUBLIC TYPES:
  ...
  type, public :: cvmix_kpp_params_type
    private
      ...
      real(cvmix_r8) :: Vtfac           ! enhancement factor of Vt by DNN
...
!BOC
    ...
    if (present(Langmuir_mixing_str)) then
       ...
       case ("DNN1")
          call cvmix_put_kpp('Langmuir_Mixing_Opt',                           &
               LANGMUIR_MIXING_DNN1, CVmix_kpp_params_user)
       case ("NONE")
          call cvmix_put_kpp('Langmuir_Mixing_Opt',                           &
    ...
    if (present(Langmuir_entrainment_str)) then
       ...
       case ("DNN2")
          call cvmix_put_kpp('Langmuir_Entrainment_Opt',                      &
               LANGMUIR_ENTRAINMENT_DNN2, CVmix_kpp_params_user)
       case ("NONE")
          call cvmix_put_kpp('Langmuir_Entrainment_Opt',                      &
...
!    if (CVmix_kpp_params_in%Langmuir_Entrainment_Opt .ne.                &
!       LANGMUIR_ENTRAINMENT_DNN2) then 
...
!BOC
    ...
    if (present(Vt_sqr_cntr)) then
      ...
      unresolved_shear_cntr_sqr = cvmix_kpp_compute_unresolved_shear(         &
                                    ...
                                    EFactor, LaSL, bfsfc, ustar,Vtfac,        &
                                    CVmix_kpp_params_user)
    end if
...
! !INPUT PARAMETERS:
    ...
    ! bfsfc: surface buoyancy flux (units: m^2/s^3)
    ! ustar: friction velocity (units: m/s)
    ! Vtfac: scalor enhancement to unresolved shear
    real(cvmix_r8), intent(in), optional :: EFactor, LaSL, bfsfc, ustar,      &
                                            Vtfac
    type(cvmix_kpp_params_type),  intent(in), optional, target ::             &
                                           CVmix_kpp_params_user
...
!BOC
    ...
    select case (CVmix_kpp_params_in%Langmuir_Entrainment_Opt)
      ...
      case (LANGMUIR_ENTRAINMENT_DNN2)
        ! Using DNN predicted enhancement to Vt (VtFac)

        if (.not.(present(Vtfac) )) then
           print*, "ERROR: you must pass in Vtfac if ",&
                "Langmuir_entrainment_str == 'DNN2'!"
           stop 1
        end if
        ! From LMD 94, Vtc = sqrt(-beta_T/(c_s*eps))/kappa^2
        Vtc = sqrt(0.2_cvmix_r8/(cvmix_get_kpp_real('c_s',                  &
               CVmix_kpp_params_in)*                                        &
              cvmix_get_kpp_real('surf_layer_ext', CVmix_kpp_params_in))) / &
              (cvmix_get_kpp_real('vonkarman', CVmix_kpp_params_in)**2)
        ...
        do kt=1,nlev
          ...
          cvmix_kpp_compute_unresolved_shear(kt) = -Cv*Vtc*zt_cntr(kt)*Vtfac*  & 
              N_cntr(kt)*ws_cntr(kt)/CVmix_kpp_params_in%Ri_crit
          if (cvmix_kpp_compute_unresolved_shear(kt).lt.                       &
              CVmix_kpp_params_in%minVtsqr) then
            cvmix_kpp_compute_unresolved_shear(kt) =                           &
                CVmix_kpp_params_in%minVtsqr
          end if
        end do
...
! Local variables
  ...
  function cvmix_kpp_compute_unresolved_shear1(zt_cntr, ws_cntr, N_iface,     &
                                            Nsqr_iface, EFactor,              &
                                            LaSL, bfsfc, ustar,Vtfac,         &
                                            CVmix_kpp_params_user)
    ...
    real(cvmix_r8), dimension(size(zt_cntr)) ::                               &
                             cvmix_kpp_compute_unresolved_shear1
    ...
    if (present(N_iface)) then
      ...
      if (present(Nsqr_iface)) then
        ...
        do kt=1,nlev
          ...
          cvmix_kpp_compute_unresolved_shear1(kt) = -Cv*Vtc*zt_cntr(kt) *    &
                            N_cntr(kt)*ws_cntr(kt)/CVmix_kpp_params_in%Ri_crit
          if (cvmix_kpp_compute_unresolved_shear1(kt).lt.                    &
              CVmix_kpp_params_in%minVtsqr) then
            cvmix_kpp_compute_unresolved_shear1(kt) = CVmix_kpp_params_in%minVtsqr
          end if
        end do

!EOC

  end function cvmix_kpp_compute_unresolved_shear1
!BOP


  function cvmix_kpp_compute_bulk_Richardson1(zt_cntr, delta_buoy_cntr,       &
                                             delta_Vsqr_cntr, Vt_sqr_cntr,    &
                                             ws_cntr, N_iface, Nsqr_iface,    &
                                             EFactor, LaSL, bfsfc, ustar,     &
                                             Vtfac,                           &
    ...
    real(cvmix_r8), dimension(size(zt_cntr)) ::                               &
                             cvmix_kpp_compute_bulk_Richardson1
!EOP
!BOC
    ...
    if (present(Vt_sqr_cntr)) then
     ...
     unresolved_shear_cntr_sqr = cvmix_kpp_compute_unresolved_shear1(         &
                                    zt_cntr, ws_cntr, N_iface, Nsqr_iface,    &
                                    EFactor, LaSL, bfsfc, ustar,Vtfac,        &
                                    CVmix_kpp_params_user)
    end if
    ...
    do kt=1,size(zt_cntr)
      ...
      denom = delta_Vsqr_cntr(kt) + unresolved_shear_cntr_sqr(kt)
      if (denom.ne.cvmix_zero) then
        cvmix_kpp_compute_bulk_Richardson1(kt) = num/denom
      else
        ! Need a better fudge factor?
        cvmix_kpp_compute_bulk_Richardson1(kt) = num*1e10_cvmix_r8
      end if
    end do

!EOC

  end function cvmix_kpp_compute_bulk_Richardson1
!BOP
