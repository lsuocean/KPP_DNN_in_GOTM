...
! !USES:
  ...
  use mod_network
  use airsea,       only: I_0, heat,precip,evap,tx,ty
...
!  method of Langmuir turbulence parameterization
   ...
   integer, parameter, public ::  KPP_LT_VR12 = 4 
   integer, parameter, public ::  KPP_LT_DNN1 = 5
   integer, parameter, public ::  KPP_LT_DNN2 = 6
...
!  variables related to DNN models
!  1. file names, imported from kpp.nml
   character(len=PATH_MAX)               ::   dnn_f1
   character(len=PATH_MAX)               ::   dnn_f2
   character(len=PATH_MAX)               ::   inp_f_mean
   character(len=PATH_MAX)               ::   inp_f_sd
   character(len=PATH_MAX)               ::   Efac_f_param
   character(len=PATH_MAX)               ::   Vtfac_f_param
!  2. normalizing parameters
   REALTYPE, dimension(:), allocatable   ::   input_mean
   REALTYPE, dimension(:), allocatable   ::   input_sd
   REALTYPE                              ::   Efac_mean
   REALTYPE                              ::   Efac_sd
   REALTYPE                              ::   Vtfac_mean
   REALTYPE                              ::   Vtfac_sd
!  3. DNN inputs
   ...
   real(rk)                              ::   dnn_input(134)
   logical                               ::   dnn_stokes
!  4. variables to store DNN outputs
   REALTYPE                              ::   tmp_fac(1)
   REALTYPE,public                       ::   Efac
   REALTYPE,public                       ::   Vtfac
   REALTYPE,public                       ::   Bfsfc
!  5. FKB model variables
   type(network_type)                    ::   model_ws
   type(network_type)                    ::   model_vt
#ifdef KPP_CVMIX
!  CVMix datatypes
...
!  'RWHGK16' - Enhancement based on Reichl et al. 2016
   ...
   namelist /kpp/                      kpp_sbl, kpp_bbl, kpp_interior,  &
                                       ...
                                       dnn_f1, dnn_f2,inp_f_mean,       &
                                       inp_f_sd,Efac_f_param,           &
                                       Vtfac_f_param,dnn_stokes
!
!-----------------------------------------------------------------------
!BOC
...
!
   ...
   allocate(input_mean(1:n_inp),stat=rc)
   if (rc /= 0) stop 'init_turbulence: Error allocating (input_mean)'
   input_mean = _ZERO_

   allocate(input_sd(1:n_inp),stat=rc)
   if (rc /= 0) stop 'init_turbulence: Error allocating (input_sd)'
   input_sd = _ZERO_
...
!  message of Langmuir turbulence parameterization
   ...
   case (KPP_LT_VR12)
      Langmuir_mixing_method = 'LWF16' !.false.
      Langmuir_entrainment_method = 'NONE' !.false.
      LEVEL3 'Langmuir turbulence parameterization   - active -   '
      LEVEL3 ' - Langmuir mixing (van Roekel et al., 2012)'
   case (KPP_LT_DNN1)
      Langmuir_mixing_method = 'DNN1' !.false.
      Langmuir_entrainment_method = 'NONE' !.false.
      LEVEL3 'Langmuir turbulence parameterization   - active -   '
      LEVEL3 ' - Langmuir mixing with dnn predicting [ws] enhancement'
      call model_ws % load(trim(dnn_f1))
      open(unit=2001, file = trim(Efac_f_param))
      read(2001,*) Efac_mean, Efac_sd
      close(2001)
       open(unit=1002, file = trim(inp_f_mean))
      do ii=1,n_inp
      read(1002,*) input_mean(ii)
      enddo
      close(1002)
      open(unit=1003, file = trim(inp_f_sd))
      read(1003,*) input_sd(1:n_inp)
      close(1003)

   case (KPP_LT_DNN2)
      Langmuir_mixing_method = 'DNN1' !.false.
      Langmuir_entrainment_method = 'DNN2' !.false.
      LEVEL3 'Langmuir turbulence parameterization   - active -   '
      LEVEL3 ' - Langmuir mixing with dnn predicting [ws]+[vt] enhancement'
      call model_ws % load(trim(dnn_f1))
      open(unit=2001, file = trim(Efac_f_param))
      read(2001,*) Efac_mean, Efac_sd
      close(2001)

      call model_vt % load(trim(dnn_f2))
      open(unit=1001, file = trim(Vtfac_f_param))
      read(1001,*) Vtfac_mean, Vtfac_sd
      close(1001)
      open(unit=1002, file = trim(inp_f_mean))
      do ii=1,n_inp
      read(1002,*) input_mean(ii)
   ...
   case (KPP_LT_RWHGK16)
      ...
      LEVEL3 'Langmuir turbulence parameterization       - active -   '
      LEVEL3 ' -  Reichl et al. 2016 (Hurricane LES tuning)'
   case default
...
!
! !INTERFACE:
   subroutine do_kpp(nlev,h0,h,rho,u,v,temp,salt,NN,NNT,NNS,SS,u_taus,  &
                    u_taub,tFlux,btFlux,sFlux,bsFlux,tRad,bRad,f)
!
! !DESCRIPTION:
...
!  temperature components at grid centers (m/s)
   REALTYPE                                      :: temp(0:nlev)
!  salinity  components at grid centers (m/s)
   REALTYPE                                      :: salt(0:nlev)
...
#ifdef KPP_CVMIX
      case (KPP_OPT_CVMIX)
         call surface_layer_cvmix(nlev,h,rho,u,v,temp,salt,NN,u_taus,&
                u_taub,tFlux,btFlux,sFlux,bsFlux,tRad,bRad,f)
#endif
      case (KPP_OPT_ROMS)
...
! !LOCAL VARIABLES:
   ...
   REALTYPE, dimension (0:nlev) :: ribt
   REALTYPE, dimension (0:nlev) :: ritp
...
!   LEVEL4 'ritp = ', ritp
!   LEVEL4 'ribt = ', ribt
!   LEVEL4 'btFlux = ', btFlux
!   LEVEL4 'bsFlux = ', bsFlux
!   LEVEL4 'bRad = ', bRad
!   LEVEL4 'rho = ', rho
   if (clip_mld) then
...
!
! !INTERFACE:
   subroutine surface_layer_cvmix(nlev,h,rho,u,v,temp,salt,NN,u_taus,   &
                u_taub,tFlux,btFlux,sFlux,bsFlux,tRad,bRad,f)
!
! !DESCRIPTION:
! In this routine all computations related to turbulence in the surface layer
...
! !LOCAL VARIABLES:
   ...
   REALTYPE                     :: bRad_cntr
   REALTYPE                     :: NN_max
   REALTYPE                     :: cff1,cff2,hz1,hz2,dR1,dR2
   REALTYPE                     :: dU1,dU2,dV1,dV2
   REALTYPE, dimension (0:nlev) :: Bflux
   REALTYPE, dimension (0:nlev) :: RiBulk
   REALTYPE, dimension (0:nlev) :: RiBulk1

!   REALTYPE, dimension (nlev*2+5) :: dnn_input 
...
!  Bflux(k) is the total buoyancy flux above the level z_r(k)
   ...
   ! DNN input array preparation (Yuan added)
   if (langmuir_method.eq.KPP_LT_DNN1.or.                              &
       langmuir_method.eq.KPP_LT_DNN2) then
     if (.not.dnn_stokes) then
      do ii = 1,(nlev/4)
        dnn_input(ii) = temp(nlev-4*(ii-1))+273.15
        dnn_input(nlev/4+ii) = salt(nlev-4*(ii-1))
      enddo
      dnn_input(2*nlev/4 + 1) = tx*rho_0
      dnn_input(2*nlev/4 + 2) = ty*rho_0
      dnn_input(2*nlev/4 + 3) = heat
      dnn_input(2*nlev/4 + 4) = I_0
      dnn_input(2*nlev/4 + 5) = -precip+evap
      dnn_input(2*nlev/4 + 6) = min(-1.964286,max(-140.0,-(abs(zsbl))))
     else
       do ii = 1,(nlev/4)
        dnn_input(ii) = temp(nlev-1-4*(ii-1))+273.15
        dnn_input(nlev/4+ii) = salt(nlev-4*(ii-1))
        dnn_input(2*nlev/4+ii) = ustokes(nlev-4*(ii-1))
        dnn_input(3*nlev/4+ii) = vstokes(nlev-4*(ii-1))
      enddo
      dnn_input(4*nlev/4 + 1) = min(1.871284,max(-1.752298,tx*rho_0))
      dnn_input(4*nlev/4 + 2) = min(1.671204,max(-1.25188,ty*rho_0))
      dnn_input(4*nlev/4 + 3) = min(212.8135,max(-551.8199,heat))
      dnn_input(4*nlev/4 + 4) = min(912.0051,max(0.0,I_0))
      dnn_input(4*nlev/4 + 5) = min(2.961e-7,max(-2.50864e-6,-precip))
      dnn_input(4*nlev/4 + 6) = min(-1.964286,max(-140.0,-(abs(zsbl))))
     endif

      dnn_input = (dnn_input - input_mean)/input_sd
      tmp_fac = model_ws % output(dnn_input)
      Efactor = 20.0*tmp_fac(1)
      EFactor = max(1.0, EFactor)
      EFactor = min(10.0, EFactor)

      if (langmuir_method.eq.KPP_LT_DNN2) then
        tmp_fac = model_vt % output(dnn_input)
        Vtfac = 20.0*tmp_fac(1)
        Vtfac = max(1.0, Vtfac)
        if (Bflux(nlev).gt.0.0) then
          Vtfac = min(10.0, Vtfac)
        endif
      endif
      CVmix_vars%Vtfac = Vtfac
   endif
   tRadSrf   =   tRad(nlev)
   bRadSrf   =   bRad(nlev)
...
!-----------------------------------------------------------------------
   ...
   if (langmuir_method.lt.KPP_LT_DNN1) then
...
!-----------------------------------------------------------------------
   ...
   do k=nlev-1,2,-1
      ...
      ! update Rref, Uref and Vref
      if (langmuir_method == KPP_LT_RWHGK16 .or. Ric<0.28) then
         u=u+ustokes
         v=v+vstokes
      endif
      if (kref < nlev) then
...
!-----------------------------------------------------------------------
   call cvmix_put(CVmix_vars, 'BoundaryLayerDepth', -zsbl)
   ksbl=1
   do k=nlev,2,-1
      if ((ksbl.eq.1).and.(z_w(k-1).lt.zsbl)) then
         ksbl = k
...
!-----------------------------------------------------------------------
   ...
   case (KPP_LT_LF17)
      efactor = min(5.0, &
           abs(cos(theta_WL))*sqrt(_ONE_+(1.5*La_SLP1)**(-2.)+(5.4*La_SLP1)**(-4.)))
   case (KPP_LT_RWHGK16)
      efactor = min(5.0, _ONE_ + _ONE_/La_SLP2)
   case default
