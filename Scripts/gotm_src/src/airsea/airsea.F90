...
!  Meteorological forcing variables
   ...
   REALTYPE, public, target            :: sw_dn,lw_dn,sw_up,lw_up
!
!  wind speed (m/s)
...
!  The short wave radiation
   ...
   if (calc_fluxes) then
      ...
      call register_input_0d(meteo_file,7,sw_dn,'downward shortwave')
      call register_input_0d(meteo_file,8,lw_dn,'downward longwave')
      call register_input_0d(meteo_file,9,precip,'precipitationrate',scale_factor=precip_factor)
#endif
...
! Yuan added (03/27/24, coare foumula, required variables)
   REAL,parameter  :: zu_1 = 10.0
   REAL,parameter  :: zt_1 = 2.0
   REAL,parameter  :: zq_1 = 2.0
   REAL,parameter  :: z_pbl_1 = 600.0
   REAL            :: w_1, airt_1, hum_1, airp_1, sst_1, sw_dn_1,lw_dn_1
   REAL            :: g_1, precip_1, sss_1, cp_1, sigH_1, u_taus_1,qe_1
   REAL            :: qh_1, Le_1, I_0_1, lw_dn_abs_1, lw_up_1 
   REAL            :: dT_skinx_1, dz_skin_1 
...
!     Calculate bulk fluxes from meteorological conditions and surface state (sst,ssu,ssv).
     if (fluxes_method .eq. 3) then
!     Use coare3.6 to calculate bulk fluxes       
        w_1 = sqrt((u10-ssu)*(u10-ssu)+(v10-ssv)*(v10-ssv))
        airt_1 = airt
        hum_1 = hum
        airp_1 = airp/100.0
        sst_1 = sst
        ...
        lw_dn_1 = lw_dn
        g_1 = g
        precip_1 = precip*3600.0*1000.0
        sss_1 = sss
        cp_1 = -10.0
        sigH_1 = -1.0
        call coare36flux_coolskin(w_1, zu_1, airt_1, zt_1, hum_1, zq_1,      &
                airp_1, sst_1, sw_dn_1, lw_dn_1, g_1, z_pbl_1, precip_1,     &
                sss_1, cp_1, sigH_1, u_taus_1, qe_1, qh_1, Le_1,             &
                I_0_1, lw_dn_abs_1, lw_up_1, dT_skinx_1, dz_skin_1)
        tx = u_taus_1*(u10-ssu)/(w_1+1.0e-8)
        ty = u_taus_1*(v10-ssv)/(w_1+1.0e-8)
        qe = -qe_1
        qh = -qh_1
        !if (abs(qe).ge.50) write(*,*) 'ck3',qe,qh,qe_1,qh_1
        evap = qh/Le_1/1000.0
        I_0 = I_0_1
        lw_up = lw_up_1
        qb = -lw_up + lw_dn
...
!     If using constant momentum flux, apply time-varying feedback from biogeochemistry to drag.
   ...
   !   write(*,*) 'ck0',have_zenith_angle,swr_method
   !   write(*,*) 'ck1',albedo_method,yearday,hh
   !   write(*,*) 'ck2',I_0,secs
      albedo = albedo_water(albedo_method,zenith_angle,yearday)
      I_0 = I_0*(_ONE_-albedo-bio_albedo)
   !   write(*,*) 'ck3',I_0,zenith_angle,albedo
   end if
...
!  This part initialises and reads in new values if necessary.
   if(time_diff(meteo_jul2,meteo_secs2,jul,secs) .lt. 0) then
      ...
      cloud = 70.0 !Yuan add (03/25/24 test)
